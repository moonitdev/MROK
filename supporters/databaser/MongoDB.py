...
class MongoDB:
    """
    기능: PyMongo 래핑(Wrapping)
    """
    def __init__(self):
        """
        기능: MongoDB 인스턴스 생성시 실행
            - conf/_config.ini 파일에서 MongoDB 접속 정보 로딩
            - 접속 정보를 이용해 MongoDB 접속에 사용할 _clinet 생성
            - _db, _collection에는 현재 사용하는 데이터베이스 및 콜렉션 명을 저장
        입력:
            - 변수명 || 의미 | 데이터 타입 | 디폴트값 | 예시
        Note:
            - _config.ini -> config.ini 필요?
        """
        config = configparser.ConfigParser()
        config.read(os.path.join(os.path.dirname(__file__), 'conf/_config.ini'))
